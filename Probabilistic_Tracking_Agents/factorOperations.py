...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        temp.update(factor.conditionedVariables())
    conditioned = set(x for x in temp if x not in unconditioned)
    joint_factor = Factor(unconditioned, conditioned, list(factors)[0].variableDomainsDict())
    for assignment in joint_factor.getAllPossibleAssignmentDicts():
        joint_prob = 1.0
        for factor in factors:
            joint_prob *= factor.getProbability(assignment)
        joint_factor.setProbability(assignment, joint_prob)
    return joint_factor
    "*** END YOUR CODE HERE ***"
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        new_factor = Factor(unconditioned, factor.conditionedVariables(), factor.variableDomainsDict())
