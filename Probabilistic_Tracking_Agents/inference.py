# inference.py
# Implements exact and approximate inference algorithms for ghost tracking in a Pacman environment.
...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for var in variables[:3]:
        variableDomainsDict[var] = pos
    for v in variables[3:]:
        dis = []
        true = [manhattanDistance(p, g) for p in pos for g in pos]
        for i in true:
            if not ((i - MAX_NOISE) < 0):
                for z in range(i - MAX_NOISE, i + MAX_NOISE + 1):
                    if z not in dis:
                        dis.append(z)
        variableDomainsDict[v] = dis
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if len(self.values()) > 0 and self.total() != 0:
            tot = self.total()
