# bustersAgents.py
# Defines agents that use probabilistic inference to locate and pursue ghosts.
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghosts = [x.argMax() for x in livingGhostPositionDistributions]
        action = None
        min_dis = float('inf')
        for act in legal:
            ghost_dists = [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), ghost) for ghost in ghosts]
            if min(ghost_dists) < min_dis:
                min_dis = min(ghost_dists)
                action = act
        return action
        "*** END YOUR CODE HERE ***"